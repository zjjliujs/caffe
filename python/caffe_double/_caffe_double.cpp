...
#define DATA_TYPE double
    ...
    BOOST_PYTHON_MODULE(_caffe_double) {
        ...
        bp::def("layer_type_list", &LayerRegistry<DATA_TYPE>::LayerTypeList);

        bp::class_<Net<DATA_TYPE>, shared_ptr<Net<DATA_TYPE> >, boost::noncopyable >("Net",
                bp::no_init)
            // Constructor
            .def("__init__", bp::make_constructor(&Net_Init<DATA_TYPE>
            ...
            // Legacy constructor
            .def("__init__", bp::make_constructor(&Net_Init_Load<DATA_TYPE>))
            .def("_forward", &Net<DATA_TYPE>::ForwardFromTo)
            .def("_backward", &Net<DATA_TYPE>::BackwardFromTo)
            .def("reshape", &Net<DATA_TYPE>::Reshape)
            .def("clear_param_diffs", &Net<DATA_TYPE>::ClearParamDiffs)
            // The cast is to select a particular overload.
            .def("copy_from", static_cast<void (Net<DATA_TYPE>::*)(const string&)>(
                        &Net<DATA_TYPE>::CopyTrainedLayersFrom))
            .def("share_with", &Net<DATA_TYPE>::ShareTrainedLayersWith)
            .add_property("_blob_loss_weights", bp::make_function(
                        &Net<DATA_TYPE>::blob_loss_weights, bp::return_internal_reference<>()))
            .def("_bottom_ids", bp::make_function(&Net<DATA_TYPE>::bottom_ids,
                        bp::return_value_policy<bp::copy_const_reference>()))
            .def("_top_ids", bp::make_function(&Net<DATA_TYPE>::top_ids,
                        bp::return_value_policy<bp::copy_const_reference>()))
            .add_property("_blobs", bp::make_function(&Net<DATA_TYPE>::blobs,
                        bp::return_internal_reference<>()))
            .add_property("layers", bp::make_function(&Net<DATA_TYPE>::layers,
                        bp::return_internal_reference<>()))
            .add_property("_blob_names", bp::make_function(&Net<DATA_TYPE>::blob_names,
                        bp::return_value_policy<bp::copy_const_reference>()))
            .add_property("_layer_names", bp::make_function(&Net<DATA_TYPE>::layer_names,
                        bp::return_value_policy<bp::copy_const_reference>()))
            .add_property("_inputs", bp::make_function(&Net<DATA_TYPE>::input_blob_indices,
                        bp::return_value_policy<bp::copy_const_reference>()))
            .add_property("_outputs",
                    bp::make_function(&Net<DATA_TYPE>::output_blob_indices,
                        bp::return_value_policy<bp::copy_const_reference>()))
            .def("_set_input_arrays", &Net_SetInputArrays<DATA_TYPE>,
                    bp::with_custodian_and_ward<1, 2, bp::with_custodian_and_ward<1, 3> >())
            .def("save", &Net_Save<DATA_TYPE>)
            .def("save_hdf5", &Net_SaveHDF5<DATA_TYPE>)
            .def("load_hdf5", &Net_LoadHDF5<DATA_TYPE>)
            .def("before_forward", &Net_before_forward<DATA_TYPE>)
            .def("after_forward", &Net_after_forward<DATA_TYPE>)
            .def("before_backward", &Net_before_backward<DATA_TYPE>)
            .def("after_backward", &Net_after_backward<DATA_TYPE>)
            .def("after_backward", &Net_add_nccl<DATA_TYPE>);
        BP_REGISTER_SHARED_PTR_TO_PYTHON(Net<DATA_TYPE>);

        bp::class_<Blob<DATA_TYPE>, shared_ptr<Blob<DATA_TYPE> >, boost::noncopyable>(
                "Blob", bp::no_init)
            .add_property("shape",
                    bp::make_function(
                        static_cast<const vector<int>& (Blob<DATA_TYPE>::*)() const>(
                            &Blob<DATA_TYPE>::shape),
                        bp::return_value_policy<bp::copy_const_reference>()))
            .add_property("num",      &Blob<DATA_TYPE>::num)
            .add_property("channels", &Blob<DATA_TYPE>::channels)
            .add_property("height",   &Blob<DATA_TYPE>::height)
            .add_property("width",    &Blob<DATA_TYPE>::width)
            .add_property("count",    static_cast<int (Blob<DATA_TYPE>::*)() const>(
                        &Blob<DATA_TYPE>::count))
            .def("reshape",           bp::raw_function(&Blob_Reshape<DATA_TYPE>))
#ifndef CPU_ONLY
            .add_property("_gpu_data_ptr",
                    reinterpret_cast<uintptr_t (Blob<DATA_TYPE>::*)()>(
                        &Blob<DATA_TYPE>::mutable_gpu_data))
            .add_property("_gpu_diff_ptr",
                    reinterpret_cast<uintptr_t (Blob<DATA_TYPE>::*)()>(
                        &Blob<DATA_TYPE>::mutable_gpu_diff))
#endif
            .add_property("data",     bp::make_function(&Blob<DATA_TYPE>::mutable_cpu_data,
                        NdarrayCallPolicies<DATA_TYPE>()))
            .add_property("diff",     bp::make_function(&Blob<DATA_TYPE>::mutable_cpu_diff,
                        NdarrayCallPolicies<DATA_TYPE>()));
        BP_REGISTER_SHARED_PTR_TO_PYTHON(Blob<DATA_TYPE>);

        bp::class_<Layer<DATA_TYPE>, shared_ptr<PythonLayer<DATA_TYPE> >,
            boost::noncopyable>("Layer", bp::init<const LayerParameter&>())
                .add_property("blobs", bp::make_function(&Layer<DATA_TYPE>::blobs,
                            bp::return_internal_reference<>()))
                .def("setup", &Layer<DATA_TYPE>::LayerSetUp)
                .def("reshape", &Layer<DATA_TYPE>::Reshape)
                .add_property("type", bp::make_function(&Layer<DATA_TYPE>::type));
        BP_REGISTER_SHARED_PTR_TO_PYTHON(Layer<DATA_TYPE>);
        ...
        bp::class_<Solver<DATA_TYPE>, shared_ptr<Solver<DATA_TYPE> >, boost::noncopyable>(
                "Solver", bp::no_init)
            .add_property("net", &Solver<DATA_TYPE>::net)
            .add_property("test_nets", bp::make_function(&Solver<DATA_TYPE>::test_nets,
                        bp::return_internal_reference<>()))
            .add_property("iter", &Solver<DATA_TYPE>::iter)
            .def("add_callback", &Solver_add_callback<DATA_TYPE>)
            .def("add_callback", &Solver_add_nccl<DATA_TYPE>)
            .def("solve", static_cast<void (Solver<DATA_TYPE>::*)(const char*)>(
                        &Solver<DATA_TYPE>::Solve), SolveOverloads())
            .def("step", &Solver<DATA_TYPE>::Step)
            .def("restore", &Solver<DATA_TYPE>::Restore)
            .def("snapshot", &Solver<DATA_TYPE>::Snapshot)
            .def("share_weights", &share_weights<DATA_TYPE>)
            .def("apply_update", &Solver<DATA_TYPE>::ApplyUpdate)
            .add_property("param", bp::make_function(&Solver<DATA_TYPE>::param,
                        bp::return_internal_reference<>()));
        BP_REGISTER_SHARED_PTR_TO_PYTHON(Solver<DATA_TYPE>);

        bp::class_<SGDSolver<DATA_TYPE>, bp::bases<Solver<DATA_TYPE> >,
            shared_ptr<SGDSolver<DATA_TYPE> >, boost::noncopyable>(
                    "SGDSolver", bp::init<string>())
                .add_property("lr", &SGDSolver<DATA_TYPE>::GetLearningRate);
        bp::class_<NesterovSolver<DATA_TYPE>, bp::bases<SGDSolver<DATA_TYPE> >,
            shared_ptr<NesterovSolver<DATA_TYPE> >, boost::noncopyable>(
                    "NesterovSolver", bp::init<string>());
        bp::class_<AdaGradSolver<DATA_TYPE>, bp::bases<SGDSolver<DATA_TYPE> >,
            shared_ptr<AdaGradSolver<DATA_TYPE> >, boost::noncopyable>(
                    "AdaGradSolver", bp::init<string>());
        bp::class_<RMSPropSolver<DATA_TYPE>, bp::bases<SGDSolver<DATA_TYPE> >,
            shared_ptr<RMSPropSolver<DATA_TYPE> >, boost::noncopyable>(
                    "RMSPropSolver", bp::init<string>());
        bp::class_<AdaDeltaSolver<DATA_TYPE>, bp::bases<SGDSolver<DATA_TYPE> >,
            shared_ptr<AdaDeltaSolver<DATA_TYPE> >, boost::noncopyable>(
                    "AdaDeltaSolver", bp::init<string>());
        bp::class_<AdamSolver<DATA_TYPE>, bp::bases<SGDSolver<DATA_TYPE> >,
            shared_ptr<AdamSolver<DATA_TYPE> >, boost::noncopyable>(
                    "AdamSolver", bp::init<string>());

        bp::def("get_solver", &GetSolverFromFile<DATA_TYPE>,
                bp::return_value_policy<bp::manage_new_object>());

        // vector wrappers for all the vector types we use
        bp::class_<vector<shared_ptr<Blob<DATA_TYPE> > > >("BlobVec")
            .def(bp::vector_indexing_suite<vector<shared_ptr<Blob<DATA_TYPE> > >, true>())
            .def("add_blob", bp::raw_function(&BlobVec_add_blob<DATA_TYPE>));
        bp::class_<vector<Blob<DATA_TYPE>*> >("RawBlobVec")
            .def(bp::vector_indexing_suite<vector<Blob<DATA_TYPE>*>, true>());
        bp::class_<vector<shared_ptr<Layer<DATA_TYPE> > > >("LayerVec")
            .def(bp::vector_indexing_suite<vector<shared_ptr<Layer<DATA_TYPE> > >, true>());
        ...
        bp::class_<NCCL<DATA_TYPE>, shared_ptr<NCCL<DATA_TYPE> >,
            boost::noncopyable>("NCCL",
                    bp::init<shared_ptr<Solver<DATA_TYPE> >, const string&>())
#ifdef USE_NCCL
                .def("new_uid", NCCL_New_Uid).staticmethod("new_uid")
                .def("bcast", &NCCL<DATA_TYPE>::Broadcast)
#endif
                /* NOLINT_NEXT_LINE(whitespace/semicolon) */
                ;
        BP_REGISTER_SHARED_PTR_TO_PYTHON(NCCL<DATA_TYPE>);
