...
class TestBlobProtoToArray(unittest.TestCase):
    ...
    def test_old_format(self):
        data = np.zeros((10,10))
        blob = caffe_double.proto.caffe_pb2.BlobProto()
        blob.data.extend(list(data.flatten()))
        shape = (1,1,10,10)
        blob.num, blob.channels, blob.height, blob.width = shape

        arr = caffe_double.io.blobproto_to_array(blob)
    ...
    def test_no_shape(self):
        ...
        with self.assertRaises(ValueError):
            caffe_double.io.blobproto_to_array(blob)
...
class TestArrayToDatum(unittest.TestCase):
    ...
    def test_label_none_size(self):
        # Set label
        d1 = caffe_double.io.array_to_datum(
            np.ones((10,10,3)), label=1)
        # Don't set label
        d2 = caffe_double.io.array_to_datum(
            np.ones((10,10,3)))
