...
from caffe_double.proto import caffe_pb2
...
def draw_net(caffe_net, rankdir, ext='png', phase=None, display_lrm=False):
    """Draws a caffe_double net and returns the image string encoded using the given
    extension.

    Parameters
    ----------
    caffe_net : a caffe_double.proto.caffe_pb2.NetParameter protocol buffer.
    ext : string, optional
...
def draw_net_to_file(caffe_net, filename, rankdir='LR', phase=None, display_lrm=False):
    """Draws a caffe_double net, and saves it to file using the format given as the
